...
The tool reuses Phase 7D embedding, Phase 7E exact search, and the extracted Phase 7F canonical resolution, content-hash freshness, requested-language-first fallback, deduplication, and ordering behavior. Request-local `menu:1` through `menu:5` IDs remain stable across distinct searches; later requested-language evidence replaces earlier fallback evidence for the same product without changing its ID. Terminal output reuses the strict Phase 7C response/source/language validator. Characterization passed before refactoring (`30 passed`) and focused Phase 7F passed after extraction (`35 passed`). The initial Phase 7A–7G regression passed (`279 passed`) and the full Python regression passed (`620 passed`, one known non-blocking joblib/loky physical-core warning with logical-core fallback). Focused review findings concerning bilingual precedence, scope-gate coverage, and live-harness bounds were fixed and retested.
...
The user-run real local V1 baseline used the cached 384-dimensional `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`, `local_files_only=True`, Phase 7D projection, and a temporary Phase 7E exact-cosine store. Its fixed 12 cases measured Hit@1 `50.0%`, Hit@3 `66.7%`, Recall@5 `88.9%`, MRR `0.6458`, and bilingual both-Hit@1 `16.7%`. English retrieval was materially stronger than Indonesian; all English gold products appeared within Top-5, while `ret-paraphrase-id` was the clearest miss. Gold labels were manually reviewed as reasonable. This is an accepted initial measurement, not a threshold, and retrieval optimization is deferred until the initial Phase 7H cycle completes.

The controlled-live evaluation passed all eight provider-backed cases, all application hard gates passed, and manual faithfulness and relevance review each scored `16/16`.

Retrieval optimization Experiment #1 adds `7d-catalog-text-v2` as a language-first bilingual projection while retaining V1 as the stable default. Requested-language category and description fields precede the other language, using only the canonical mapping `makanan ↔ food`, `minuman ↔ drink`, and `camilan ↔ snack`. Its verified result is Hit@1 `83.3%`, Hit@3 `91.7%`, Recall@5 `97.2%`, MRR `0.8917`, and bilingual both-Hit@1 `66.7%`; V1's verified numbers remain unchanged.
