...
//    FE-B mengaktifkan trust tepat satu hop HANYA pada production, sesuai
//    topologi satu trusted edge dari FE-A. Node production wajib tidak dapat
//    diakses langsung, dan edge wajib mengganti forwarding headers client.
//    Development/test tetap tidak mempercayai proxy.
...
function clearLimitsForTests() {
  if (process.env.NODE_ENV !== 'test') throw new Error('Rate-limit reset hanya tersedia dalam test.');
  buckets.clear();
}
...
module.exports = { checkLimit, clearLimitsForTests };
