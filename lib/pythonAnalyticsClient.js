...
function isModelComparisonResponse(value) {
  if (value === null || Array.isArray(value) || typeof value !== 'object' ||
      !hasExactKeys(value, ['evaluation', 'experimental_inference', 'models'])) return false;
  const evaluation = value.evaluation;
  if (evaluation === null || Array.isArray(evaluation) || typeof evaluation !== 'object' ||
      !hasExactKeys(evaluation, ['dataset_identity', 'end_date', 'metric_unit', 'start_date']) ||
      !isIsoDate(evaluation.start_date) || !isIsoDate(evaluation.end_date) ||
      evaluation.start_date > evaluation.end_date ||
      evaluation.dataset_identity !== 'sari_rasa_ml_synthetic_transactions_v2' ||
      evaluation.metric_unit !== 'next_day_total_quantity') return false;
  const expected = [
    ['hist_gradient_boosting', 'production'],
    ['mlp_10_16_1_relu', 'experimental'],
    ['previous_week', 'benchmark'],
  ];
  if (!Array.isArray(value.models) || value.models.length !== expected.length) return false;
  for (let index = 0; index < expected.length; index += 1) {
    const model = value.models[index];
    if (model === null || Array.isArray(model) || typeof model !== 'object' ||
        !hasExactKeys(model, ['mae', 'name', 'rmse', 'role', 'type']) ||
        !isNonEmptyString(model.name) || model.type !== expected[index][0] ||
        model.role !== expected[index][1] || !isNonNegativeNumber(model.mae) ||
        !isNonNegativeNumber(model.rmse)) return false;
  }
  const inference = value.experimental_inference;
  if (inference === null || Array.isArray(inference) || typeof inference !== 'object' ||
      !hasExactKeys(inference, ['artifact_version', 'data_through', 'forecast_date', 'model_family', 'predicted_quantity', 'role']) ||
      !isIsoDate(inference.forecast_date) || !isIsoDate(inference.data_through) ||
      !isNonNegativeNumber(inference.predicted_quantity) ||
      inference.model_family !== 'experimental_mlp' || inference.artifact_version !== '1.0' ||
      inference.role !== 'experimental') return false;
  const cutoff = new Date(`${inference.data_through}T00:00:00Z`);
  cutoff.setUTCDate(cutoff.getUTCDate() + 1);
  return cutoff.toISOString().slice(0, 10) === inference.forecast_date;
}
...
function getModelComparison() {
  return requestAnalytics('analytics/forecast/model-comparison', isModelComparisonResponse);
}
