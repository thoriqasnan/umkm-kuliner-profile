...
// Cookie transport policy follows the repository's existing NODE_ENV pattern:
// production is HTTPS-only, while local development/test HTTP remains usable.
// Keep these options centralized so login and logout cannot drift apart.
function getSessionCookieOptions({ includeMaxAge = false } = {}) {
  const options = {
    httpOnly: true,
    sameSite: 'lax',
  ...
  if (includeMaxAge) options.maxAge = MAX_AGE_MS;
  return options;
}
...
module.exports = { sign, verify, getSessionCookieOptions, COOKIE_NAME, MAX_AGE_MS };
